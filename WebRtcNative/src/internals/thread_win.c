...
void CRYPTO_MUTEX_lock_read(CRYPTO_MUTEX *lock) {
  //AcquireSRWLockShared((SRWLOCK *) lock);
	EnterCriticalSection((CRITICAL_SECTION *)lock->handle);
}
...
void CRYPTO_MUTEX_unlock_read(CRYPTO_MUTEX *lock) {
  //ReleaseSRWLockShared((SRWLOCK *) lock);
	LeaveCriticalSection((CRITICAL_SECTION *)lock->handle);
}
...
void CRYPTO_MUTEX_cleanup(CRYPTO_MUTEX *lock) {
  /* SRWLOCKs require no cleanup. */
	DeleteCriticalSection((CRITICAL_SECTION *)lock->handle);
}
...
void CRYPTO_STATIC_MUTEX_lock_read(struct CRYPTO_STATIC_MUTEX *lock) {
  //AcquireSRWLockShared(&lock->lock);
	...
	EnterCriticalSection((CRITICAL_SECTION *)lock->lock.Ptr);
}
...
void CRYPTO_STATIC_MUTEX_unlock_read(struct CRYPTO_STATIC_MUTEX *lock) {
  //ReleaseSRWLockShared(&lock->lock);
	LeaveCriticalSection((CRITICAL_SECTION *)lock->lock.Ptr);
}
